    ...
    if (s->show_mode == SHOW_MODE_WAVES) {
        ...
        }
        {
            ...
            for (y = 0; y < s->height; y++) {
                double w = 1 / sqrt((double)nb_freq);	// TODO:sqrt arg double???
...
static int queue_picture(VideoState *is, AVFrame *src_frame, double pts1, int64_t pos)
{
    ...
    if (vp->bmp) {
        ...
        av_picture_copy(&pict, (AVPicture *)src_frame,
                        (PixelFormat)src_frame->format, vp->width, vp->height);
#else
...
static int video_thread(void *arg)
{
    ...
    int last_w = 0;
    int last_h = 0;
    enum PixelFormat last_format = (PixelFormat)-2;
...
static void toggle_audio_display(VideoState *is)
{
    ...
    is->show_mode = (ShowMode)((is->show_mode + 1) % SHOW_MODE_NB);
    fill_rectangle(screen,
...
static void event_loop(VideoState *cur_stream)
{
    ...
    for (;;) {
        ...
        case FF_REFRESH_EVENT:
            video_refresh((VideoState *)event.user.data1);
