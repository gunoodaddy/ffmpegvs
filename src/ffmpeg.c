...
char _av_error_string[AV_ERROR_MAX_STRING_SIZE] = {0, };
char _av_ts_make_string[32] = {0, };
...
static void sub2video_update(InputStream *ist, AVSubtitle *sub)
{
    ...
    int     dst_linesize;
    int i;
    int64_t pts = av_rescale_q(sub->pts, _AV_TIME_BASE_Q, ist->st->time_base);

    if (!ref)
        return;
    dst          = (int8_t *)ref->data    [0];
                            ...
                            AVSubtitle *sub)
{
    ...
    for (i = 0; i < nb; i++) {
		...
		AVRational temp = { 1, 1000 };
        sub->pts               += av_rescale_q(sub->start_display_time, temp, _AV_TIME_BASE_Q);
        ...
        pkt.pts  = av_rescale_q(sub->pts, _AV_TIME_BASE_Q, ost->st->time_base);
        pkt.duration = av_rescale_q(sub->end_display_time, temp, ost->st->time_base);
        if (enc->codec_id == AV_CODEC_ID_DVB_SUBTITLE) {
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if ((is_last_report || !ost->finished) && ost->st->pts.val != AV_NOPTS_VALUE)
            pts = FFMAX(pts, av_rescale_q(ost->st->pts.val,
                                          ost->st->time_base, _AV_TIME_BASE_Q));
    }
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    if (decoded_frame->pts != AV_NOPTS_VALUE) {
        ist->dts = ist->next_dts = ist->pts = ist->next_pts = av_rescale_q(decoded_frame->pts, avctx->time_base, _AV_TIME_BASE_Q);
    ...
    if (decoded_frame->pts != AV_NOPTS_VALUE) {
		AVRational temp = {1, ist->st->codec->sample_rate};
...
static void print_sdp(void)
{
    char sdp[2048];
    int i;
    AVFormatContext **avc = (AVFormatContext **)av_malloc(sizeof(*avc) * nb_output_files);
                                    ...
                                    AVCodecContext *avctx)
{
    ...
    ost->forced_kf_pts   = (int64_t *)av_malloc(sizeof(*ost->forced_kf_pts) * n);
    ...
    for (i = 0; i < n; i++) {
        ...
        ost->forced_kf_pts[i] = av_rescale_q(t, _AV_TIME_BASE_Q, avctx->time_base);
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                if (!codec->sample_aspect_ratio.num) {
                    ...
                    ost->st->sample_aspect_ratio =
                        ...
                        ist->st->codec->sample_aspect_ratio : temp;
                }
...
//                    ost->frame_rate = ist->st->avg_frame_rate.num ? ist->st->avg_frame_rate : (AVRational){25, 1};
			...
			case AVMEDIA_TYPE_AUDIO:
				{
					AVRational temp = { 1, codec->sample_rate };
					codec->sample_fmt     = (AVSampleFormat)ost->filter->filter->inputs[0]->format;
					codec->sample_rate    = ost->filter->filter->inputs[0]->sample_rate;
					codec->channel_layout = ost->filter->filter->inputs[0]->channel_layout;
					codec->channels       = av_get_channel_layout_nb_channels(codec->channel_layout);
					codec->time_base      = temp;
				}
				break;
            case AVMEDIA_TYPE_VIDEO:
                codec->time_base = av_inv_q(ost->frame_rate);
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->encoding_needed) {
            ...
            if (dec && dec->subtitle_header) {
                ...
                ost->st->codec->subtitle_header = (uint8_t *)av_mallocz(dec->subtitle_header_size + 1);
...
static int process_input(int file_index)
{
    ...
    if(!ist->wrap_correction_done && input_files[file_index]->ctx->start_time != AV_NOPTS_VALUE && ist->st->pts_wrap_bits < 64){
        int64_t stime = av_rescale_q(input_files[file_index]->ctx->start_time, _AV_TIME_BASE_Q, ist->st->time_base);
