...
void init_opts(void)
{
    ...
    if(CONFIG_SWSCALE)
        sws_opts = sws_getContext(16, 16, (PixelFormat)0, 16, 16, (PixelFormat)0, SWS_BICUBIC,
                              NULL, NULL, NULL);
...
static void print_program_info(int flags, int level)
{
    ...
    av_log(NULL, level, "%s version ", program_name);
...
static unsigned get_codecs_sorted(const AVCodecDescriptor ***rcodecs)
{
    ...
    while ((desc = (const AVCodecDescriptor *)avcodec_descriptor_next(desc)))
        nb_codecs++;
...
{
    //enum PixelFormat pix_fmt;	// WIN32
	int pix_fmt;
...
#   define sws_isSupportedOutput(x) 0
    ...
    for (pix_fmt = 0; pix_fmt < PIX_FMT_NB; pix_fmt++) {
        ...
        printf("%c%c%c%c%c %-16s       %d            %2d\n",
               sws_isSupportedInput ((PixelFormat)pix_fmt)      ? 'I' : '.',
               sws_isSupportedOutput((PixelFormat)pix_fmt)      ? 'O' : '.',
